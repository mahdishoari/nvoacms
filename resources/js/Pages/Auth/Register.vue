...
<template>
    ...
    <ui-card>
        <form class="space-y-6" action="#" @submit.prevent="submit">
            ...
            <div>
                <ui-input type="text" name="name" id="name" v-model="form.name" :error="form.errors.name"
                    label="نام شما" placeholder="نام کامل شما"></ui-input>
                <ui-input type="email" name="email" id="email" v-model="form.email" :error="form.errors.email"
                    label="پست الکترونیکی" placeholder="آدرس پست الکترونیکی شما"></ui-input>
                <ui-input type="password" name="password" id="password" v-model="form.password"
                    :error="form.errors.password" label="رمز عبور" placeholder="••••••••"></ui-input>
                <ui-input type="password" name="password_confirmation" id="password_confirmation"
                    ...
                    label="تکرار رمز عبور" placeholder="••••••••"></ui-input>
            </div>
            <div class="flex items-start">
                <Link href="/forgot-password" class="ms-auto text-sm text-blue-700 hover:underline dark:text-blue-500">
                رمز را فراموش کرده اید؟</Link>
            </div>
            <button type="submit" :disabled="form.processing"
                class="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">ایجاد
                حساب</button>
